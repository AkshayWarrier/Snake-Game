...
highscore_font = pg.font.Font('Quicksand-VariableFont_wght.ttf',25)
def highscore_text():
    highscore_text = highscore_font.render("Highscore:"+str(max(highscore)), True, (0, 0, 0))
    win.blit(highscore_text, (0, 45))
    
lives_font = pg.font.Font('Quicksand-VariableFont_wght.ttf',25)
def lives_text():
    lives_text = lives_font.render("Lives:"+str(lives), True, (0, 0, 0))
    win.blit(lives_text, (0, 25))    
...
def badfood(x,y):
    badfoodimg = pg.image.load('worm.png')
    win.blit(badfoodimg,(x,y))
...
badfoodx = random.randrange(32,608,32)
badfoody = random.randrange(32,608,32)
...
while run:
    ...
    if (badfoodx,badfoody) == (left[0],top[0]):
        badfoodx = random.randrange(32, 608, 32)
        badfoody = random.randrange(32, 608, 32)
        lives -= 1
